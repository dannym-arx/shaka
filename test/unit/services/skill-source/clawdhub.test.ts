...
describe("ClawdhubSourceProvider", () => {
  ...
  describe("resolveLatestVersion", () => {
    test("resolves latest version for installed skill", async () => {
      const provider = createClawdhubProvider({
        resolveLatest: async () => ok("2.0.0"),
      });
    ...
    test("passes slug from skill.source", async () => {
      let receivedSlug = "";
      const provider = createClawdhubProvider({
        resolveLatest: async (slug) => {
          receivedSlug = slug;
          return ok("1.0.0");
        },
      });

      await provider.resolveLatestVersion({
        source: "my-skill",
        provider: "clawdhub",
        version: "0.9.0",
        subdirectory: null,
      ...
      expect(receivedSlug).toBe("my-skill");
    });

    test("propagates resolve errors", async () => {
      const provider = createClawdhubProvider({
        resolveLatest: async () => {
          return { ok: false as const, error: new Error("Network error") };
        },
      });
